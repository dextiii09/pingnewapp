...
const VerificationModal = ({ onClose, userRole, onUpdateUser, userId }: { onClose: () => void; userRole: UserRole; onUpdateUser: (data: Partial<User>) => void; userId: string }) => {
  ...
  const handleDocUpload = async () => {
    ...
    try {
        const path = `users/${userId}/verification/doc_${Date.now()}_${docFile.name}`;
        const url = await api.uploadFile(docFile, path);
        onUpdateUser({ docUrl: url, verificationStatus: VerificationStatus.PENDING });
  ...
  const handleVideoUpload = async () => {
    ...
    try {
        const path = `users/${userId}/verification/video_${Date.now()}_${videoFile.name}`;
        const url = await api.uploadFile(videoFile, path);
        onUpdateUser({ introVideoUrl: url });
    ...
    <div className="h-full w-full flex flex-col relative overflow-y-auto no-scrollbar bg-white dark:bg-[#050505]">
      ...
      <div className="px-6 pt-12 pb-6 flex items-center justify-between sticky top-0 z-20 bg-white/90 dark:bg-[#050505]/90 backdrop-blur-md transition-colors">
        <div className="flex items-center gap-3">
           {/* Add App Logo to Dashboard Header */}
           <img src={APP_LOGO} className="w-10 h-10 rounded-xl shadow-lg shadow-pink-500/20" alt="Ping" />
           <div className="flex flex-col">
              <span className="text-xs font-bold text-gray-400 dark:text-white/40 uppercase tracking-widest mb-0.5">Welcome back</span>
      ...
      <AnimatePresence>
         {showVerificationModal && <VerificationModal onClose={() => setShowVerificationModal(false)} userRole={user.role} onUpdateUser={onUpdateUser} userId={user.id} />}
      </AnimatePresence>
    </div>
  );
};
