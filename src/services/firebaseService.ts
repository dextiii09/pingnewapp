...
class FirebaseService {
  ...
  async login(email?: string, password?: string): Promise<User> {
    ...
    try {
      ...
      try {
        ...
        if (email === 'admin@ping.com' && (authError.code === 'auth/user-not-found' || authError.code === 'auth/invalid-credential' || authError.code === 'auth/invalid-login-credentials')) {
      ...
      if (userDoc.exists()) {
        ...
        if (fetchedUser.role === UserRole.ADMIN && fetchedUser.avatar !== APP_LOGO) {
  ...
  async updateUserProfile(userId: string, data: Partial<User>): Promise<void> {
    ...
    if (this.currentUser?.id !== userId && this.currentUser?.role !== UserRole.ADMIN) {
  ...
  async getAdminStats(): Promise<AdminStats> {
    ...
    // Check permission
    if (this.currentUser?.role !== UserRole.ADMIN) {
        // Return zeros if not admin to prevent data leakage, though server rules should block this
        return { totalUsers: 0, split: { business: 0, influencer: 0 }, revenue: 0, activeMatches: 0, pendingVerifications: 0 };
    }
  ...
  async seedDatabase(): Promise<void> {
    this.checkConfig();
    if (this.currentUser?.role !== UserRole.ADMIN) return; // Protect Seeding
